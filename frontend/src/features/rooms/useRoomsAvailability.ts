...
export function useRoomsAvailability(userId: string, run: (action: () => Promise<void>) => Promise<void>, setMessage: (msg: string) => void) {
  ...
  const loadModeratorReservations = async (adminHeaders: HeadersInit, filter: { status: string; roomId: string; from: string; to: string }) => {
    await run(async () => {
